...
public class XTHandler extends Handler {
    ...
    public void handleMessage(MessageDto messageDto) {
        ...
        if ((AQBusinessConstant.AT + AQBusinessConstant.XT_ID).equals(messageExt)) {
            //拷贝一份消息
            ...
            //修改消息内容 去掉@AI
            aiMessageDto.setMessageContent(messageDto.getMessageContent().replace((AQBusinessConstant.AT + AQBusinessConstant.XT_NAME), ""));

            mqSendingAgent.xt(aiMessageDto);
        }else {
            //继续传递
            if (handler != null) {
