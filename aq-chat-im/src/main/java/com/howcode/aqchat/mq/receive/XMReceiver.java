...
public class XMReceiver extends AbstractAISpaceReceiver implements InitializingBean {
    ...
    public void initConsumer() {
        try {
            ...
            defaultMQPushConsumer.setConsumerGroup(AQChatMQConstant.ConsumerGroup.XM_CONSUMER_GROUP);
            defaultMQPushConsumer.setConsumeMessageBatchMaxSize(1);
            defaultMQPushConsumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
            defaultMQPushConsumer.subscribe(AQChatMQConstant.MQTopic.XM_TOPIC, "*");
            defaultMQPushConsumer.setMessageListener((MessageListenerConcurrently) (messageExtList, consumeConcurrentlyContext) -> {
                for (MessageExt messageExt : messageExtList) {
                    ...
                    LOGGER.info("mq收到消息[XM 文本转图片模型]:{}", msgStr);
                    threadPoolUtil.submitTask(() -> {
                        ...
                        if (!msgStr.isEmpty()) {
                            ...
                            try {
                                AIResult aiResult = aiService.textToImage(messageDto.getMessageContent());
                                ...
                                aiMessageDto.setMsgType(MsgTypeEnum.IMAGE.getCode());
            ...
            LOGGER.info("[文本转图片模型]订阅消息成功");
        } catch (MQClientException e) {
            LOGGER.error("文本转图片模型 订阅失败", e);
        }
    }
